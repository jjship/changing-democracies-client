...
import mask from "./dziura_po_cieniu.png";
import bgMobile from "./tlo_mobile.jpg";
import maskMobile from "./dziura_po_cieniu_720.png";
...
import { languageAbbreviations, languages, turquoise, black, darkRed, yellowBrown, pink, altLayouts, layouts, translations, INACTIVITY_THRESHOLD, violet, Language, Translations, getTranslation, Layouts, grey, darkGrey } from "@/lib/photobooth/const";
...
let VIDEO: TYPE
  ...
  openBold = p.loadFont('fonts/OpenSans-Bold.ttf');
    ...
    p.setup = () => {
			...
			nextButton = new DavButton("Next", p.width / 2 - 200 / 2, p.height / 2, 200,null, p);
			finishButton = new DavButton("Finish", p.width / 2 - 240 / 2, 5 * p.height / 6, 240, null, p);
			repeatButton = new DavButton("Repeat picture", p.width / 2 - 400 / 2, 2 * p.height / 3, 400, 5, p);
			pictureButton = new DavButton("Take a picture", p.width / 2 - 400 / 2, 5 * p.height / 6, 400, null, p);
			startButton = new DavButton("Start", p.width / 2 - 200 / 2, 2 * p.height / 3, 200, null, p);
			...
			let startY = (p.height - (buttonHeight * languages.length + buttonSpacing * (languages.length - 1))) / 2;

			for (let i = 0; i < languages.length; i++) {
				let language = languages[i];
				let btnY = startY + i * (buttonHeight + buttonSpacing);
				languageButtons.push(new BtnLang(language, startX, btnY, buttonWidth, p));
			}
    };
    ...
    p.draw = () => {
			...
			p.text(getTranslation(currentLayout, "Make your own poster", translations), p.width/2, p.height/8);

			p.fill(darkRed);
			p.textSize(32);
			p.text(getTranslation(currentLayout, "Express your feelings about Democracy", translations), p.width/2, p.height/2);
			p.pop();
			startButton.display(pink,
				darkRed,
				mPressed,
				p,
				stage,
			...
			p.textSize(24);
	p.textAlign(p.CENTER, p.CENTER);
			p.text(getTranslation(currentLayout, "What is your name?", translations), p.width/2, p.height/8);
			p.textSize(36);
			p.fill(darkRed);
	p.textAlign(p.CENTER, p.TOP);
			p.text(displayText, p.width/2, p.height/4);
			p.pop();
			nextButton.display( pink,
				darkRed,
				mPressed,
				p,
				stage,
	...
	p.textAlign(RIGHT, TOP);
			...
			underlineData.forEach(underlineItem => {
				...
				p.stroke(darkRed);
				p.strokeWeight(90);
				p.line(underlineItem.startX, underlineItem.y, underlineItem.endX, underlineItem.y);
				p.noStroke();
				p.fill(yellowBrown);
				p.triangle(underlineItem.endX-r/2+p.cos(a1)*r, underlineItem.y+p.sin(a1) *r, underlineItem.endX-r/2+p.cos(a2)*r, underlineItem.y+p.sin(a2)*r, underlineItem.endX-r/2+p.cos(a3)*r, underlineItem.y+p.sin(a3)*r);
				p.pop();
			}
			);
			...
			textData.textLinesData.forEach(textLineData => {
				p.text(textLineData.text, 40, textLineData.y+16, p.width - 80, p.height / 8);
			}
			);
			...
			p.textSize(24);
	p.textAlign(p.CENTER, p.CENTER);
			p.text(getTranslation(currentLayout, "Write your statement", translations), p.width / 2, p.height / 4);
			...
			textData.textLinesData.forEach(textLineData => {
				...
				p.stroke(darkRed);
				p.strokeWeight(90);
				p.line(startX, textLineData.y + lineHeight - 16, endX, textLineData.y + lineHeight - 16); // Adjust y position for underline
				p.noStroke();
				p.fill(yellowBrown);
				p.triangle(endX-r/2+p.cos(a1)*r, textLineData.y + lineHeight - 16+p.sin(a1) *r, endX-r/2+p.cos(a2)*r, textLineData.y + lineHeight - 16+p.sin(a2)*r, endX-r/2+p.cos(a3)*r, textLineData.y + lineHeight - 16+p.sin(a3)*r);
			}
			);
	...
	p.textAlign(p.RIGHT, p.TOP);
			...
			textData.textLinesData.forEach(textLineData => {
				p.text(textLineData.text, 40, textLineData.y-8, p.width - 80, p.height / 8);
	...
	p.textAlign(p.RIGHT, p.TOP);
		...
		function updateKeys(layout: keyof Layouts) {
			keys = [];
			...
			layouts[keyboard].forEach((row, rowIndex) => {
				...
				rowKeys.forEach((key, keyIndex) => {
					...
					keys.push(new Key(keyIndex * keyWidth, yPosition, keyWidth, keyHeight, key, p));
				}
				);
			}
			);
	...
	p.textAlign(p.CENTER, p.CENTER);
		...
		function backButton() {
			...
			if (p.mouseX >= marginB && p.mouseX <= marginB*2+sW/2 + wb && p.mouseY >= marginB && p.mouseY <= marginB + hb) {
				p.fill(violet);
				p.stroke(pink);
				if (mPressed) {
					stage = 0;
			...
			p.noStroke();
			p.triangle(marginB*2+p.cos(a1)*hb/2, marginB*2 +p.sin(a1)*hb/2, marginB*2+p.cos(a2)*hb/2, marginB*2+p.sin(a2)*hb/2, marginB*2+p.cos(a3)*hb/2, marginB*2+p.sin(a3)*hb/2);
			p.pop();
		}
